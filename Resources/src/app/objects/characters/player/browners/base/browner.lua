...
function browner:charge()
    ...
    if self.charging_ then --&& global.current_weapon[? "object"] == buster){
        ...
        if self.charge_timer_ == 20 then
            ...
            self.sound_effects_[self.charge_power_] = cc.audio.play_sfx("sounds/sfx_buster_charging_mid.mp3", false)
        ...
        elseif self.charge_timer_ >= 60 then
            ...
            if self.charge_timer_ % 18 == 0 then
                self.sound_effects_[self.charge_power_] = cc.audio.play_sfx("sounds/sfx_buster_charging_high.mp3", false)
...
function browner:fire()
    ...
    if self.charge_power_ == "low" then
        cc.audio.play_sfx("sounds/sfx_buster_shoot.mp3", false)
