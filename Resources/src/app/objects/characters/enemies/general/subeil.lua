...
function mob:walk()
    ...
    if self.on_ground_ then
        ...
        local player_bbox = self.player_:bbox()
        local projected_bbox_right = self.kinematic_body_:bbox()
        local projected_bbox_left = self.kinematic_body_:bbox()

        projected_bbox_right.width = 128
        projected_bbox_left.x = projected_bbox_left.x - 128
        projected_bbox_left.width = 128
...
--                self.player_:getPositionY() >= self.kinematic_body_.bbox_[1].y + self.kinematic_body_.bbox_[1].height
            if cc.rectIntersectsRect(player_bbox, projected_bbox_right) or
                cc.rectIntersectsRect(player_bbox, projected_bbox_left)    
            then
