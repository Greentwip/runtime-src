...
function sprite:load_action(action, prepend_action, base_name)
    ...
    if action.animation.forever then
        local animation_sequence = cc.Sequence:create(cc.Animate:create(animation_frames), cc.CallFunc:create(function() self.cycles_ = self.cycles_ + 1 end))

        animation = cc.RepeatForever:create(animation_sequence)
    else
        local animation_sequence = cc.Sequence:create(cc.Animate:create(animation_frames), cc.CallFunc:create(function() self.cycles_ = self.cycles_ + 1 end))
